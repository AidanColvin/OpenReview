...
declare global {
  interface Window {
    ...
    addFromSearch: (idx: number, btn: HTMLElement) => void;
    lastResults: Article[];
  }
}
...
function renderImportList(): void {
  ...
  list.innerHTML = state.articles.map(a => `<div style="padding:8px 10px; border-bottom:1px solid #f0ece8;"><p style="font-size:12px; font-weight:600; color:#1a1a1a; margin:0;">${esc(a.title)}</p></div>`).join('');
}
...
window.addFromSearch = (idx: number, btn: HTMLElement) => {
  ...
  btn.style.color = '#1a6b42';
  btn.style.borderColor = '#1a6b42';
};

async function handleSearch() {
  const q = (el('crossref-input') as HTMLInputElement).value.trim(); if (!q) return;
  el('crossref-results').innerHTML = '<p style="padding:1rem;text-align:center;font-size:0.8rem;">Searching...</p>';
...
function bindEvents(): void {
  ...
  // KEYBOARD: Enter to start review
  const startInput = el('input-review-title');
  startInput.onkeydown = (e) => { if (e.key === 'Enter') el('btn-create-review').click(); };
